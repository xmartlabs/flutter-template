...
abstract class HiveBaseSource<Key, Model> implements LocalStorage<Key, Model> {
  ...
  @override
  Future<Model> putElement(Key key, Model response) async =>
      withBox((box) async {
        await box.put(key, dbParser(response));
        return response;
      });
  ...
  Future<List<Model>> putAllElements(Map<Key, Model> entries) =>
      withBox((box) async {
        await box.putAll(entries.mapValues((entry) => dbParser(entry.value)));
        return getElements();
      });
  ...
  @override
  Future<Model?> getElement(Key key) => withBox((box) async {
    final data = box.get(key);
    return data == null ? null : modelParser(data);
  });

  @override
  Stream<Model?> getElementStream(Key key) async* {
    final box = await getBox();
    yield await getElement(key);
    yield* box.watch(key: key).map((event) => modelParser(event.value));
  }

  @override
  Future<List<Model>> getElements() => withBox(
    (box) => Future.value(box.values.map((e) => modelParser(e)).toList()),
  );
  ...
  Stream<List<Model>> getElementsStream() async* {
    ...
    yield* box.watch().flatMap((event) => Stream.fromFuture(getElements()));
  }
}
